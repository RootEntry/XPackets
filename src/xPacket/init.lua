...
local BUILTIN_COMPRESSION = true
	...
	for k, v in pairs(_Packets) do
		if v[1].FlowType == "Data" then
			if v[1].Protocol == "ClientToServer" then
				...
				delay(0, function()
					while true do
						...
						if os.clock() - T1 > v[1].TimeToLive then
							timedout = true
							warn(("Packet #%s timed out. \nHeader: %s"):format(v[1].Id, TableUtil.EncodeJSON(v[1])))
							table.insert(PACKET_LOSS_BUFFER, #PACKET_LOSS_BUFFER + 1, 0x1)
							break
						end
			...
			elseif v[1].Protocol == "ServerToClient" then
				...
				response = ReplicatedStorage.PacketSender_Func:InvokeClient(v[1].Destination, v)
